...
```
L7  PRESENTATION    AGI safety amber + frontier black (Open Design)
L6  ORCHESTRATION   Hermes sub-context (Kimi K2.6 / DeepSeek V3.5 / local)
L5  DOMAIN MCP      care-membrane-mcp, ai-self-audit-mcp, deepfake-detector-mcp
L4  AGENT MEMORY    agentmemory (v0.9.26) — shared mode
L3  KNOWLEDGE GRAPH Cognee subgraph — scope: AGI safety research, alignment papers, capability evaluations, frontier-model incidents
L2  VERSIONED HIST  Memoria (v0.4.0) — namespace "agisafe"
...
```bash
git clone https://github.com/CSOAI-ORG/agisafe-hive
cd agisafe-hive
...
`https://agisafe.ai/.well-known/agent-card.json` — see `agent-card.json`.
...
Research hub; csoai.org traffic absorber. Or flip for $10-25k.
